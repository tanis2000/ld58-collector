...
namespace GameBase.Navigation
{
    // Pathfinder taken from PowerQuest by PowerHoof
// Horrible pathfinding hacked from old Crawl code and influenced by PolyNav2D
    public class Pathfinder
    {
        public class PathPoly
        {
            ...
            public Transform
                m_transform =
                    null; // usually the colliders transform, stored for efficiency. But can optionally only set this and not the polygon collider
        ...
        public Vector2[] FindPath(Vector2 pointStart, Vector2 pointEnd)
        {
            ...
            if (EvaluateDijkstra(nodeStart, nodeEnd))
            {
                ...
                while (node != nodeStart) // iterate to node after the first
                {
                    ...
                    if ((result[0] - node.m_position).sqrMagnitude >
                        1.0f) // Don't insert positions that are too close (within 1.0f)
                        result.Insert(0, node.m_position);
                }
            }
            else
        ...
        public Vector2 FindNextPoint(Vector2 pointStart, Vector2 pointEnd)
        {
            ...
            if (EvaluateDijkstra(nodeStart, nodeEnd))
            {
                ...
                while (node.m_previous != nodeStart // iterate to node after to the first
                       && (pointStart - node.m_previous.m_position).sqrMagnitude >
                       1.0f) // as long as you're not already too close
                {
                    node = node.m_previous;
                }
        ...
        public void DrawDebugLines()
        {
            ...
            {
                DrawDebugPoly(poly.m_vertsInflated,
                    (poly == m_mainPoly) ? Color.green : (poly.m_enabled ? Color.red : Color.grey));
            }
        ...
        PathPoly AddPolygon(PolygonCollider2D collider, Transform transform, bool isMain,
            Vector2[] pointsOverride = null)
        {
            m_dirty = true;
            //
        ...
        bool EvaluateDijkstra(PathNode startNode, PathNode endNode)
        {
            ...
            while (queue.Count > 0)
            {
                ...
                for (int i = 0;
                     i < currNode.m_links.Count;
                     ++i) // Loop through links. Add unvisted ones to queue if cost is good
                {
        ...
        public static void ReversePoly(Vector2[] poly)
        {
            for (int i = 0; i < poly.Length * 0.5f; ++i)
                Utils.Utils.Swap(ref poly[i], ref poly[poly.Length - 1 - i]);
        }
    }
...
// More efficient way of checking lots interseting lines of against a single line. Just set the vector that's changed
    public class LineIntersector
    {
        ...
        public bool Calculate(Vector2 secondLineStart, Vector2 secondLineEnd)
        {
            ...
            float r = (((m_start1.y - secondLineStart.y) * end2MinusStart2.x) -
                       ((m_start1.x - secondLineStart.x) * end2MinusStart2.y)) * denom;
            float s = (((m_start1.y - secondLineStart.y) * m_end1MinusStart1.x) -
                       ((m_start1.x - secondLineStart.x) * m_end1MinusStart1.y)) * denom;
        ...
        public static bool FindIntersection(Vector2 start1, Vector2 end1, Vector2 start2, Vector2 end2,
            out Vector2 result)
        {
            // Adapted from http://stackoverflow.com/questions/1119451/how-to-tell-if-a-line-intersects-a-polygon-in-c
